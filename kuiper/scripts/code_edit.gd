...
func code_request_code_completion():
	# Basic C/C++ keywords
	var keywords = ["if", "else", "while", "for", "do", "switch", "case", "break", "continue", "return", "class", "struct"]
	var types = ["int", "char", "float", "double", "void", "bool", "unsigned", "signed", "long", "short"]
	var preprocessor = ["#include", "#define", "#ifdef", "#ifndef", "#endif", "#pragma"]
	var std_funcs = ["printf", "scanf", "malloc", "free", "strlen", "strcpy", "memcpy"]
	...
	# Add types
	for type in types:
		add_code_completion_option(CodeEdit.KIND_CLASS, type, type + " ")
	
	# Add preprocessor directives
	for prep in preprocessor:
		add_code_completion_option(CodeEdit.KIND_MEMBER, prep, prep + " ")
	
	# Add standard functions
	for func_name in std_funcs:
		add_code_completion_option(CodeEdit.KIND_FUNCTION, func_name, func_name + "()")
	
	# Add common PLAIN_TEXTs
	add_code_completion_option(CodeEdit.KIND_PLAIN_TEXT, "main", "int main(int argc, char *argv[]) {\n\t\n\treturn 0;\n}")
	add_code_completion_option(CodeEdit.KIND_PLAIN_TEXT, "if", "if () {\n\t\n}")
	add_code_completion_option(CodeEdit.KIND_PLAIN_TEXT, "for", "for (int i = 0; i < n; i++) {\n\t\n}")
